class UsersController < ApplicationController
  ...
  def node
    if @current_user.is_offer? || @current_user.is_admin
      @nodes = Node.all
    else
  ...
  def check_in
    if @checked
      ...
      @current_user.transfer_enable += 104_857_600
      @current_user.save
  ...
  def avatar
    if avatar_params && @current_user.update(avatar: avatar_params[:avatar])
  ...
  def human_readable_time(int_time)
    if int_time == 0
      ...
      time_ago_in_words(Time.at(int_time))
    end
  end
